...
$(document).ready( function () {
	...
	var oTable3 = $('#error-table').dataTable();
	new FixedHeader( oTable2, { "bottom": true } );
...
echo "<form>";
echo "Owner: <select name='OWNER' VALUE='OWNER' onChange=\"document.location='main.php?page=$page&START_DATE=$START_DATE&END_DATE=$END_DATE&STATUS=$STATUS&CONTROL_ID=$CONTROL_ID&OWNER='+this.options[this.selectedIndex].value;\">";
owner_drop_down($_GET['OWNER']);
echo "</select>";


echo "Status: <select name='OWNER' VALUE='OWNER' onChange=\"document.location='main.php?page=$page&START_DATE=$START_DATE&END_DATE=$END_DATE&OWNER=$OWNER&CONTROL_ID=$CONTROL_ID&STATUS='+this.options[this.selectedIndex].value;\">";
status_drop_down($_GET['STATUS']);
echo "</select>";
echo "Start date: <input type='text' id='datepickerstart' value='". $_GET['START_DATE'] ."'>";
echo "End Date: <input type='text' id='datepickerend' value='" . $_GET['END_DATE'] . "'>";
...
ca on cc.control_id = ca.control_id  where cc.OWNER like '$owner' and cc.status like '$status' and cc.CREATE_DT between to_date('$start_date','MM/DD/YYYY') and to_date('$end_date','MM/DD/YYYY')";
            ...
            echo "<div style='height:350px;'><table id='top-table' class='dataTable'><thead><tr class='tablesorter-stickyHeader kiketable-colsizable '>";
			...
			<th class='top-header-cell col2' kiketable-th>STATUS</th>
...
foreach ($res as $col) {
    ...
    foreach ($col as $item) {
        ...
        if($i==2){
		echo " <td class='top-body-cell col" . $i . "'>\n";
		$STATUS_CHANGE=$res[$cnum]['CONTROL_ID'] . "_STATUS_CHANGE;
		echo "<select name='CHANGE' VALUE='CHANGE' onChange=\"document.location='main.php?page=$page&START_DATE=$START_DATE&END_DATE=$END_DATE&STATUS=$STATUS&CONTROL_ID=$CONTROL_ID&OWNER=$OWNER&" . $res[$cnum]['CONTROL_ID'] . "_STATUS_CHANGE='+this.options[this.selectedIndex].value;\">";
        status_codes($res[$cnum]['CONTROL_ID'],$res[$cnum]['status']);
		echo "</td></select>";
	    // function for status goes here passing control_change as variable
		///echo $control_change;
		}
	
		else{
echo "    <td class='top-body-cell col" . $i . "'><div><a href='main.php?page=$page&START_DATE=$START_DATE&END_DATE=$END_DATE&OWNER=$OWNER&STATUS=$STATUS&CONTROL_ID=" . $res[$cnum]['CONTROL_ID'] . "'>". $item . "</a></div></td>\n";
...
echo "<a href='main.php?OWNER=$OWNER&STATUS=$STATUS&START_DATE=$START_DATE&END_DATE=$END_DATE&CONTROL_ID=$CONTROL_ID&page=". $i . "'>". $i*100 ."</a> "; 
};
...
$e_sql = "SELECT c.last_job_name, m.message, c.create_dt FROM cnsl_file_control c, cnsl_mainlog m WHERE c.control_id = m.control_id AND c.status IN (72,73,76,19,99,13,131,132,133,281,201)";
            $doc3 = ociparse($conn, $e_sql);
                ociexecute($doc3);
                //$nrows = oci_fetch_all($doc,$res);
                             oci_fetch_all($doc3, $res3, 0, -1, OCI_FETCHSTATEMENT_BY_ROW);
							 
echo "<div style='height:350px;'><table id='error-table' class='dataTable'><thead><tr>";
