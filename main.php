...
where cc.OWNER like '$owner' and cc.status like '$status' and cc.CREATE_DT between to_date('$start_date','MM/DD/YYYY') and to_date('$end_date','MM/DD/YYYY') order by control_id desc";

            $doc = ociparse($conn, $m_sql);
                ociexecute($doc);
				
                //$nrows = oci_fetch_all($doc,$res);
                             oci_fetch_all($doc, $res, $start_from,50, OCI_FETCHSTATEMENT_BY_ROW);
...
echo "<a href='main.php?OWNER=$OWNER&STATUS=$STATUS&START_DATE=$START_DATE&END_DATE=$END_DATE&CHANGE_CONTROL=NO&NEW_STATUS=$STATUS&CONTROL_ID=$CONTROL_ID&page=". $i . "'>". $i."</a> "; 
};
